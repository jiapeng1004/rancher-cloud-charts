...
## rancher-cloud-charts 说明（镜像 · Rancher）

- **国内镜像**：`values.yaml` 中 **`image.*.repository`** 默认使用华为云 SWR 「第三方镜像」路径  
  `swr.<region>/ddn-k8s/docker.io/<Docker Hub path>`（示例区为 **`cn-north-4`**，请按控制台实际为准）。
- **MinIO**：由上游 **`cgr.dev/chainguard/minio*`** 改为 **`minio/minio`** 与 **`minio/mc`**，与同前缀镜像一样走 Docker Hub 代理；若要恢复 Chainguard 可自行改回原仓库/tag。
- **Rancher 表单**：见根目录 **`questions.yaml`**（JWT、Realtime、`deployment.db`、`ingress`、常用镜像与组件开关）。复杂字段仍建议 «编辑 YAML»。
